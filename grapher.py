...
class ObjectTree:	
	...
	def connect(self, plot=False, export_df=False):
		...
		side_object_list = []
		for id in nearest_dest_ids_hori:
			if id == -1:
				side_object_list.append('NULL')
			else:
				side_object_list.append(df.loc[id, 'Object'])
...
class Graph:
	...
	def _pad_text_features(self, feat_arr):
		...
		if self.max_nodes > feat_arr.shape[0]:
			target[:feat_arr.shape[0], :min(self.max_nodes, feat_arr.shape[1])] = feat_arr
		elif self.max_nodes < feat_arr.shape[0]:
			target[:self.max_nodes, :min(self.max_nodes, feat_arr.shape[1])] = feat_arr[:self.max_nodes, :]
		else: 
			target = feat_arr
...
if __name__ == "__main__":
	print(os.getcwd())
	df = pd.read_csv('D:/New_Research/project_NLP/scan004.csv')
	img = cv2.imread('D:/New_Research/project_NLP/dataGNN/img_train/scan004_page_1.png', 0)
